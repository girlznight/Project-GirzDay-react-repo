...
import NoteBg       from "../../assets/sticky-note.png";   // 포스트잇 배경 하나로 통일
...
export default function Post() {
  ...
  const [copied,   setCopy]   = useState(false); // 공유 알림
  ...
  const delPost = async () => {
    ...
    const latest = await fetch(
      "http://localhost:5000/post?_sort=createdAt&_order=desc"
    ).then(r => r.json());
    ...
    <div className="relative min-h-screen bg-[#fcfcf8] p-4 overflow-hidden">
      ...
      {cOpen && (
        <AlertPopup onClose={() => setCOpen(false)}>
          <div
            className="relative w-[380px] h-[380px] p-6"
